...
shinyServer(function(input, output,session){
  ...
  output$plot <- renderPlot({
    sdata<-score()
    #E:\Atrial fibrillation\data\1000Genome_Atrial fibrillation.txt
    d1000<-read.table("E:/Atrial fibrillation/data/1000Genome_Atrial fibrillation.txt",head=T,sep="\t",stringsAsFactors = F)
...
score.score <- reactive({
  sdata<-score()	
  d1000<-read.table("E:/Atrial fibrillation/data/1000Genome_Type 2 diabetes.txt",head=T,sep="\t",stringsAsFactors = F)
...
score.score()
  ...
  output$range <- renderText({
    calc.range <- reactive({
      ...
      selectdata<-sdata[1,c("Sum(OR)")]
      if(!is.null(selectdata)){
        if(selectdata==0){print("VCF文件未包含心房颤动相关位点")}else{
        d1000<-read.table("E:/Atrial fibrillation/data/1000Genome_Atrial fibrillation.txt",header=TRUE,sep="\t",stringsAsFactors = F)
        score2=(which.min(abs(sort(d1000[,2])-selectdata))[1]-0.5)/dim(d1000)[1]
        if(!is.na(score2)){
        if(score2<0.05){print("心房颤动风险较低")}else if(score2>0.95){print("心房颤动风险偏高")}else{print("心房颤动风险处于正常范围")}
   }
  }
}
...
version3<-function(tabdata){
  version3database<-read.table("E:/Atrial fibrillation/data/Atrial fibrillation.txt",head=T,sep="\t",stringsAsFactors = F)
  cg<-c("C","G")
  at<-c("A","T")
...
vcf2tab <- function(vcf){
  ...
  GT1 <- substr(info,1,3)
  GT2 <- ""
  database4<-read.table("E:/Atrial fibrillation/data/Atrial fibrillation.txt",head=T,sep="\t",stringsAsFactors = F)
  DP <- ""
  AD <- ""
  PL <- ""
  dbSNP <- ""
